...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    async function fetchStats() {
      const { data } = await supabase.from('tb_screenings').select('*');
      if (data) {
        setRecentData(data.slice(0, 10)); // Ambil 10 terbaru
        setStats({
          total: data.length,
  ...
  return (
    <div className="p-8 bg-slate-50 min-h-screen">
      ...
      <div className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm">
        <div className="p-6 border-b border-slate-100 font-bold text-slate-800">10 Riwayat Terakhir</div>
